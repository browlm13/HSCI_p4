...
//modify weights and points for given order
void get_xw(int order, vector<double> &xi, vector<double> &w){
  ...
  if (order == 1){
    ...
    cout << "Illegal order value..." << endl;
  }

  //update passed vectors
  xi.insert(xi.begin() , xi_n , xi_n + order) ; 
  w.insert(w.begin() , w_n , w_n + order) ; 
